...
def main():
    ...
    with driver.session() as session:
        ...
        features, labels = [], []
        for row in result:
            features.append(row["embedding"])
            labels.append(row["labels"])

        features_matrix = numpy.asarray(features)
        labels_matrix = scipy.sparse.csr_matrix(numpy.asarray(labels))

        result = session.run("""
        MATCH (:Label)
        RETURN count(*) AS count""")
        labels_count = result.peek()["count"]
    #
    # print(labels_matrix, features_matrix)
    # labels_count = labels_matrix.shape[1]
